...
export default function DetailDirection() {
  ...
  const minuteDuration = Math.round(dataRoute.duration / 60).toFixed(
    1
  ) as unknown as number;
  const kmDistance = Math.round(dataRoute.distance / 1000).toFixed(
    1
  ) as unknown as number;
    ...
    <>
      ...
      >
        ...
        <Typography>
          ...
          gian chuyến đi: {isNaN(minuteDuration) ? '' : minuteDuration} {'phút'}
        </Typography>
        <Typography>
          <Iconify icon="emojione:motorway" sx={{ paddingTop: '3px' }} /> Tổng quãng đường
          chuyến đi: {isNaN(kmDistance) ? '' : kmDistance} {'km'}
        </Typography>
